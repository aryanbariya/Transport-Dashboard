...
import { grainColumns } from "./columns";
import { useGrains } from "@/hooks/use-grains";

export function GrainTable() {
    const columns = React.useMemo(() => grainColumns, []);
    ...
    const { data: response, isLoading, isError } = useGrains({
    ...
    if (isError) {
        return (
            <div className="flex h-[400px] items-center justify-center rounded-md border border-dashed text-destructive">
                Failed to load grain data. Please try again later.
            </div>
        );
    }
